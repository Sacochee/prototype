  ...
  data.forEach((item) => {
    ...
    doc.content.nodesBetween(from, to, (node, pos) => {
      ...
      if (text!.includes(defText)) {
        ...
        // Si def contient target → mettre target en rouge
        if (defText.includes(target)) {
          const idx = text?.indexOf(target) || -1;
          if (idx !== -1) {
            tr = tr.addMark(
              pos + idx,
